...
# taken from https://www.r-bloggers.com/2014/01/estimating-a-nonlinear-time-series-model-in-r/
...
y <- simnlts(2000, 0.7, -2.2, -0.8, 1, 300)
...
train_df <- data.frame(y = c(y)) %>%
  dplyr::mutate( lag_1 = dplyr::lag(y, 1),
                 ...
                 lag_8 = dplyr::lag(y, 8),
                 lag_9 = dplyr::lag(y, 9),
                 lag_10 = dplyr::lag(y, 10),
                 lag_11 = dplyr::lag(y, 11),
                 time_index = seq_len(length(y)))
train <- train_df[1:1800,]
test <- train_df[1800:2000,]
  ...
  for( observation in available_obs ) {
    ...
    for( i in observation:min(observation+5, nrow(oob_obs)) ) {
      ...
      curr_test <- data.frame( y = NA,
                               ...
                               lag_8 = curr_test[["lag_7"]],
                               lag_9 = curr_test[["lag_8"]],
                               lag_10 = curr_test[["lag_9"]],
                               lag_11 = curr_test[["lag_10"]],
